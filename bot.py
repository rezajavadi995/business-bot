...
async def render_admin_panel(update, context, data, *, edit=False, query=None):
    """
    Single source of truth for admin panel UI.
    """
    if context.user_data.get("panel_rendered_for_update") == update.update_id:
        return
    context.user_data["panel_rendered_for_update"] = update.update_id
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data and q.data.startswith("user:"):
        ...
        return
    if not is_admin(uid,data): return
    if q.data in {"menu:admin", "toggle:active", "admin:selfbot", "admin:welcome_toggle", "admin:db_export"} or q.data.startswith("panel:") or data.get("panel_open"):
        ...
        return await render_admin_panel(update, context, data, edit=True, query=q)
    if q.data=="admin:features": await q.edit_message_text("فیچرها", reply_markup=create_features_keyboard(data))
